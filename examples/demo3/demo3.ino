...
*/
//------------------------------------CONFIGURATION START------------------------------------
//#define rp2040_zeroTiny // <-- escull placa board
#ifdef rp2040_zeroTiny
  #define sdCsPin 5
#else
  #define sdCsPin 17
#endif
...
class sdBrowserDisplay : public screenMenu{
    ...
    public:
        ...
        sdBrowserDisplay();
        void setHasMores() override;
        virtual bool refreshMenu() override;
        virtual void run(unsigned int index) override;
        virtual bool pushDn() override;
        virtual bool pushUp() override;
...
void sdBrowserDisplay::run(unsigned int index){
    ...
    File entry;///home/garfius/.platformio/packages/framework-arduinopico/libraries/SDFS/src/SDFS.h
    time_t tmpTime;
    ...
    if (entry){
        if(entry.isDirectory()){
            ...
            menuSystemOverTtyP->printDotLeaders("Name",entry.name());
            menuSystemOverTtyP->printDotLeaders("FullName",entry.fullName());
            tmpStr = String(entry.size());
            menuSystemOverTtyP->printDotLeaders("Size:",tmpStr.c_str());
            tmpTime = entry.getCreationTime();
            menuSystemOverTtyP->printDotLeaders("CreationTime:",ctime(&tmpTime));
            tmpTime = entry.getLastWrite();
            menuSystemOverTtyP->printDotLeaders("LastWrite:",ctime(&tmpTime));
            entry.close();
        }
    }else{
...
void sdBrowserDisplay::leave(){
    ...
    if(currentFolder.length() == 0){
        ...
        menuSystemOverTtyP->userTty->print("Card type:");
